...
downloader = function (argument, callback) {
	if (reURL.test(argument)) { //console.log(this); throw new Error ('this');
		downloader.downloadFromURL(argument, callback);

	} else if (rePath.test(argument)) { //console.log(this); throw new Error ('this');
		downloader.downloadFromPath(argument, callback);

	} else {
		// return an error
		callback(new Error("URL or path to file is not properly formatted. Please try again."));
	}
};
...
//downloader.downloadFromURL = downloadFromURL;
//downloader.downloadFromPath = downloadFromPath;
