...
fileWriter.prototype.seek_absolute_start_index = function (callback) {
	return this.pieceIndex * this.pieceLength;
};

fileWriter.prototype.seek_absolute_end_index = function (callback) {
...
fileWriter.prototype.get_affected_files_indices = function (absolute_start, absolute_end, callback) {
	...
	minFile = this.binarySearch(absolute_start, 0, numberOfFiles, callback); console.log('minFile: ' + minFile);
	maxFile = this.binarySearch(absolute_end, minFile, numberOfFiles, callback); console.log('maxFile: ' + maxFile);

	for (var i = minFile; i < maxFile + 1; i ++) {
		affected_files.push(i);
	}
	console.log(affected_files);
...
fileWriter.prototype.binarySearch = function (absolute_index, min, max, callback) {
	...
	if (max - min > 1) {
		if (currentIndex > absolute_index) {
			return this.binarySearch(absolute_index, min, mid - 1);
		}

		if (currentIndex < absolute_index) {
			return this.binarySearch(absolute_index, mid + 1, max);
		}
	}
