...
Peer.prototype.send_bitfield = function() {
	...
	// console.log(final_buffer); throw new Error ('bitfield buffer');
...
Peer.prototype.request_piece_from_PWC = function () {
	// ask for piece index from PeerWireProtocol
...
Peer.prototype.request_piece = function	(next) {
	var pieceLength = this.piece.pieceLength,
		requestedAllBlocks = false;
	...
	console.log('Requesting piece ' + this.pieceIndex + ' block ' + i + ' from ' + this.ip + ':' + this.port);

	if (i <= pieceLength) { 
		// requesting blocks 0, 1, ..., n - 1
		this.inTransitPieceData = new Buffer(pieceLength);
		this.send_request(this.pieceIndex, i, Math.pow(2,14))

	} else if (requestedAllBlocks == false ) {
		// requesting last block (which can be shorder than 2^14 bytes)
		this.send_request(this.pieceIndex, pieceLength - i, Math.pow(2, 14));
		requestedAllBlocks = true;

	} else {
		// 1. check if SHA1 sum of downloaded content
		// corresponds to the info from the torrent file
		console.log(utils.sha1(this.data.piece));
		console.log(this.piece.pieceTable[this.pieceIndex]);

		throw new Error ('Downloaded a full piece');

		// 2. write piece to disc

		// 3. let PWP know that this peer is ready again.
	}
};
