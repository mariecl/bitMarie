...
*
* Consumes request for a block from a peer
*
...
Peer.prototype.parse_request = function () {
	...
	throw new Error ('Read a parse request'); // still never happened
...
Peer.prototype.send_unchoke = function () {
	...
	var length_buffer, id_buffer;

	length_buffer = new Buffer ('1');
...
Peer.prototype.request_piece_index = function () {
	// ask for piece index from PeerWireProtocol
	var pieceIndex = this.peerWireProtocol.attribute_piece_index(this);

	// send request to a peer
	this.request_piece(pieceIndex);
};

Peer.prototype.request_piece = function	(pieceIndex) {
	// Call this.send_request(pieceIndex, begin, end) until full piece is received
