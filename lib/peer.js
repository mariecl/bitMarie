...
Peer.prototype.openSocket = function (callback) {
	...
	socket.on('data', function (data) {
		console.log('Data from ' + this.ip + ':' + this.port);
		this.data = Buffer.concat([this.data, data]);

		if (!this.received_handshake) {
			this.readHandshake(callback);
...
Peer.prototype.parseMessages = function () {
	...
	if (this.data.readUInt32BE(this.offset) === 0) {
		this.parse_keep_alive();

	} else {
		var message_id = this.data[this.offset + 5]; console.log(message_id);
		console.log('readUInt8: ' + this.data.readUInt8(this.offset + 5));
		console.log('readUInt32BE: ' + this.data.readUInt32BE(this.offset + 5));
...
Peer.prototype.parse_bitfield = function () {
	...
	throw new Error ('Received bitfield response');
