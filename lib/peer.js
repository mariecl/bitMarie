...
var red = clc.redBright,
	bright_green = clc.greenBright,
	dark_green = clc.green,
	orange = clc.xterm(216),
	blue = clc.blue;
...
*/
function Peer (ip, port, pieceInfo, torrentSession, callback) {
	if (net.isIP(ip) == 0) {
...
Peer.prototype.connect = function (callback) {
	if (this.isConnected()) {
		return callback(new Error('Already connected to that peer.'));
	}

	var socket = net.createConnection(this.port, this.ip, function () {
		console.log(bright_green('Connected to ' + this.ip + ':' + this.port));
		this.socket = socket;
		this.sendHandshake();
		setTimeout((function () {if (this.connected) {this.send_keep_alive();}}).bind(this), 60000);
	}.bind(this));
	...
	socket.on('end', function() {
		console.log(red(this.ip + ':' + this.port + ' ended our connection'));
		socket.destroy();
		this.socket = undefined;
	}.bind(this))
}
...
Peer.prototype.parse_choke = function () {
	console.log('Received choke from ' + this.ip + ':' + this.port + ' -> peer choked me');
	this.peer_choking = 1;
...
Peer.prototype.parse_piece = function (length) {
	console.log(dark_green('Received piece from ' + this.ip + ':' + this.port)); // throw new Error('actually received a block');
...
Peer.prototype.parse_cancel = function () {
	console.log(orange('Received cancel from ' + this.ip + ':' + this.port));
...
Peer.prototype.parse_port = function () {
	console.log(orange('Received port from ' + this.ip + ':' + this.port));
...
Peer.prototype.send_bitfield = function() {
	...
	var length_buffer = new Buffer (4),
		...
		encodedBitfield = encodeBitField(this.piece.bitfield),
...
Peer.prototype.send_have = function (pieceIndex) {
	console.log(blue('Sending have ' + pieceIndex + ' to ' + this.ip + ':' + this.port));
	var length_buffer = new Buffer (4),
		id_buffer = new Buffer (1),
		pieceIndex_buffer = new Buffer (4),
		final_buffer;

	length_buffer.writeUInt32BE(5, 0);
	id_buffer.writeUInt8(4, 0);
	pieceIndex_buffer.writeUInt32BE(pieceIndex, 0);

	final_buffer = Buffer.concat([length_buffer, id_buffer, pieceIndex_buffer]); console.log(final_buffer);
...
Peer.prototype.request_piece_from_PWC = function () {
	this.pieceIndex = this.torrentSession.attribute_piece_index(this);
...
Peer.prototype.request_piece = function	(lastDownloadedBlock) {
	...
	for (var i = 0; i < 20; i ++) {
		...
		if (utils.sha1(this.inTransitPieceData)[i] != this.piece.piecesTable[this.pieceIndex * 20][i]) {
			throw new Error(red('There was a problem while downloading'))
			// this.request_piece(); 
	...
	this.torrentSession.advertise(this.pieceIndex);
...
function encodeBitField (bitfield) {
	var binaryString = '',
		encodedBitfield = [];

	for (var i = 0; i < bitfield.length; i ++) {
		binaryString += bitfield[i];
