...
Peer.prototype.request_piece = function	(lastDownloadedBlock) {
	...
	if (i < this.piece.pieceLength) { console.log('case 1');
		// requesting blocks 0, 1, ..., n - 1
		this.send_request(this.pieceIndex, i, blockLength);
		return;
	}

	if (this.piece.pieceLength % blockLength != 0) { console.log('case 2');
	...
	console.log('case 3'); console.log(this.inTransitPieceData);
	...
	for (var i = 0; i < 20; i ++) {
		...
		if (this.inTransitPieceData[i] != this.piece.piecesTable[this.pieceIndex * 20][i]) {
			request_piece();
	...
	this.piece.bitfield[pieceIndex] = 1;
	this.piece.transitfield = 0;

	//4. have PWC send have messages to all connected peers

	//5. ask for a new piece index to download
	this.request_piece_from_PWC();
	return;
};
