...
function trackerConnection (torrentObject) {
	...
	this.info_hash = computeInfoHash(torrentObject.info);
	...
	*/
	...
	request({
		...
		new PeerWireProtocol(bencode.decode(body), config.info_hash, config.peer_id);
		//new PeerWireProtocol(bencode.decode(body, 'utf8'), config.info_hash, config.peer_id);
	})
}
...
*/
function computeInfoHash (infoDictionnary) {
	var hash = sha1(bencode.encode(infoDictionnary));
	var info_hash = "";
